...
import type { WalletTransaction, RewardSpin } from "../types/api"
...
export async function getWalletData(
  authUserId: string,
): Promise<{ success: boolean; balance?: number; transactions?: WalletTransaction[]; error?: string }> {
  try {
...
): Promise<{ success: boolean; spin?: RewardSpin; error?: string }> {
  try {
    // Get the user's ID from the database
    const { data: userData, error: userError } = await supabase
      .from("users")
    ...
    const reward = calculateReward(tier, pointsToSpend)

    // Begin transaction
    const { data, error } = await supabase.rpc("create_spin_with_transaction", {
      user_id_param: userData.user_id,
      points_spent_param: pointsToSpend,
